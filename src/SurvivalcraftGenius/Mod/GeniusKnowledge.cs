...
public sealed class GeniusKnowledgeStore(string directory)
{
    ...
    [
        ...
        5. **use_bucket(x,y,z)** —— 取水/倒水,这是移动水的唯一办法(dig_block/place_block 都不行)。
           ...
           **开渠的正确姿势**:用 dig_block 在田边或田中间挖出**一格四面有壁的坑**,把水倒进坑里——
           水被四壁挡住不会漫出去,耕地 3 格内有水就湿润。敞口的地方倒水会冲毁作物、耕地退回泥土,
           工具会直接拒绝并让我先挖沟。
           附近 64m 搜不到水**不等于世界上没水**:teleport 去远处(海边/湖)装满桶再传回来。
        ...
        **成熟标准(照各方块自己的掉落表,提前割纯亏,因为株一样没了)**:
        ...
        硬门槛和坑:
        - **作物头顶光照必须 ≥9,否则一点都不长**——湿润和肥料都救不了。
          但这不是"这块地不能种",**插一根火把就解决**:till_soil 收尾时如果光照不够、我身上又有火把,
          会自动插在**田边**(火把压在耕地上会把它退回泥土,所以只能插旁边)。没火把就先 craft 一根。
        - **绝不在田里或田正下方挖矿/挖竖井**:会把耕地一起挖穿。要挖矿先 teleport 到离田几十米外。
