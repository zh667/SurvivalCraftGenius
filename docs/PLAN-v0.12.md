...
> **这是唯一的计划文件。** 要动手时看这里,尤其是 **§六 轮次安排**。
> 另外两份是它的**参考资料**,不是计划:
> `docs/GAP-ANALYSIS-NUMEN.md`(为什么要做这些)、
> `docs/BORROW-FROM-SC-MODS.md`(移植的具体参数和代码位置)。
>
> 2026-08-13。基于差距报告第二版 + 一次全量代码审计 + 两个 SC 模组的反编译。
...
> **实测更正(2026-08-13,R2 做完后)**:下面预测的"省 61%"是**错的**,实际是 **-34%**
> (6,446 → 4,284 tok/步,每步计费 12,978 → 10,829,降 17%)。
>
> **错在哪**:我把中英文按**字符数**换算了。中文 1.90 tok/字 对英文 0.44 tok/字符,
> 单价差 4.3 倍不假 —— 但**一个汉字要 4~5 个英文字符才说得清同样的意思**,长度把单价优势吃掉大半。
> 原文 5,879 字符 → 译文 9,369 字符。
>
> 连带更正一条推论:上一份差距报告说"Numen 的提示词便宜是因为用英文写",不全对 ——
> **它便宜主要是因为它说得少**(4,825 字符 vs 我们 9,369),语言只占一部分。
>
> 另外实测到的一件事:**译文写啰嗦了照样白搭**。第一版译文 11,716 字符只省了 17%,
> 又紧缩了一轮(删解释性废话、不删规则)才到 -34%。省 token 的真功夫是**少说**,不是换语言。
...
| 级 | 做什么 | 预估 | **实测** |
|---|---|---|---|
| 1 | D1a 提示词改英文 | 降 31% | **降 17%**(12,978 → 10,829 tok/步) |
| 2 | D1b 删重复正文 | 再降一些 | 并入上面那一轮紧缩;单独删只值 60 tok |
| 3 | **A4 常驻模式** | 砍掉"步数"本身 | 未做 |

前两级降的是**单价**,第三级降的是**数量**。

**做完 R2 之后的教训**:单价这条路的天花板比我以为的低得多。提示词从 6,446 压到 4,284 tok
已经是"每条规则都还在、只是不说废话"的极限了,再压就要删规则、拿行为冒险。
**第三级(常驻模式)才是真正的大头** —— 一轮农田维护今天要 4+ 步 ≈ 52,000 tok,
常驻模式下是 0。降单价 17% 和把整轮步数砍掉,不是一个量级的事。
...
11 个条目打包成 **5 轮**,每轮一个可发布的版本。分轮的依据是**改哪些文件**,
不是"感觉上像一类" —— 同一轮内的条目彼此不冲突,可以一次做完。

### 为什么 D2 从第 9 位提到第 1 位

A1、A4、A2、A3 **四条都要往工具表里加工具**,而加工具今天意味着改两个同样的地方:
`GeniusToolDefinition.cs` 的目录,和 `GeniusPlayerComponent.cs` 里那个 421 行的 switch。

**这就是所有并行的瓶颈。** 先做 D2 把 switch 拆成"一个工具一个文件",
后面四条就变成各写各的文件,互不打架;不先做,它们只能排队,而且每条都要
在一个已经 1594 行、还在继续长的类里动刀。

代价说清楚:**第一轮玩家看不到任何变化。** 所以第一轮捎带两条小的真修复,并且尽量短。
你要是更想先看到游戏里变好,把 R1 和 R3 对调即可 —— 后果只是 R3/R4 各自要多改一次 switch。

### 五轮

| 轮 | 版本 | 内容 | 主题 | 游戏里看得见吗 |
|---|---|---|---|---|
| **R1** | v0.12.0 | **D2** 拆上帝类 + **C5** craft 查地标 + **D3** teleport 加代价 | 地基 | **部分能** —— 见下 |
| **R2** | v0.12.1 | **D1a** 提示词改英文 → **D1b** 删重复正文 → **C3** 知识索引 + **C4** 本能自述 | 省钱 | **账单降 15%**,行为微变 |
| **R3** | v0.12.2 | **A1** todowrite + **C1** 改替换式 + **A2** task_status/stop | 大脑:状态有地方放 | **是** —— 直接打转圈 |
| **R4** | v0.12.3 | **A4** 农活常驻模式 + **C2** 远程武器 + 弹道提前量 | 身体:移植两个模组 | **是** —— 打猎、种田 |
| **R5** | v0.12.4 | **A3** 预制建筑 + **D4** 知识常量挪嵌入资源 | 建造 | **是** —— 房子好看且一致 |

### 批一(v0.12.0)在游戏里到底看得见什么 【2026-08-13 更正】

我在计划里把 R1 标成"几乎看不见"、R2 标成"看不见",**这两个说法都偏了**。
拆 switch 确实看不见,但同批的其它几条是能感觉到的:

| 看得见的 | 表现 |
|---|---|
| teleport 加了代价 | 它会**多走路少传送**;20 格以内直接回"这点距离我走过去",刚传送过会说"还要等 X 秒" |
| craft/smelt 查地标 | 要合成时**直接走回记住的那张工作台**,不再原地卡一下扫一圈 |
| 知识索引进提示词 | 需要查攻略时**少一次往返** —— 以前它得先空调一次 read_knowledge 才知道有哪些文件 |
| 工具名大小写兜底 | 模型偶尔把 `goto` 写成 `Goto`,以前整个回合作废,现在照常执行 |
| **账单** | 每步 12,978 → 11,042 tok(**-15%**),盖一间房 40 步省下的是真金白银 |
| recipe_trap 那条修复 | 你问"石镐怎么做",它会**去查你说的那个词**再告诉你这游戏没有,而不是自作主张answer石锤 |

看不见的只有:拆 switch 本身、`ToolTableTests`、`LandmarkMemory.Nearest`。
**所以批一不需要你专门去玩,但装上之后账单和走路方式会立刻不一样。**
...
| 批 | 版本 | 合并谁 | 为什么能合 |
|---|---|---|---|
| **批一** | v0.12.0 | **R1 + R2** | 文件完全不重叠(R1 动派发层,R2 动提示词);而且 **R2 靠 `ToolBench` 判定好坏,根本不需要你进游戏** |
| **批二** | v0.12.1 | **R3 + R5** | 大脑状态(todowrite/任务协议)和建筑预制件是两个互不相干的区域,出问题按症状就能分清是哪边 |
| **批三** | v0.12.2 | **R4** | 只能单独,原因见下 |

### 三条不能合并的理由

**R3/R4/R5 都不能和 R1 同批。** 它们都要加工具,而 R1 正是在改"加工具的那个地方"。
更要紧的是:**R1 是纯重构,一旦和新功能同批发布,出了问题就分不清是重构搞坏的还是新功能的 bug**
—— 215 个测试能挡住编译期和逻辑错误,挡不住"实机行为变了"。重构必须自己走一版。

**R4 必须排在 R3 之后,不能与之并行。** A4 的常驻模式要回答"谁占用身体、什么时候让位",
而 A2 的任务协议正在重新定义这件事,两者改的是 `ComponentGeniusBrain` 里同一段生命周期。
先做 R3,A4 才有一个定型的协议可以挂进去;并行做就是两个人改同一个概念。

**R2 是唯一可以"你在玩的时候我同时做"的一轮**,因为它的验收标准是基准分,不是实机表现。

### 每轮内部可以一起做,但批与批之间不要并行

**轮内**:同一轮的条目改的是不同文件(R1 除外,R1 本身就是重构),可以一次性做完再一起测。

**轮间**:技术上 R2(提示词)和 R4(身体)碰的文件不重叠,确实能并行。**但不要这么做** ——
playtest 13 和 15 两次都是拿 N-1 版的日志来分析的,并行发版只会让"你现在装的到底是哪个版本"
更难说清。**一轮一个版本号,装上、玩、回日志,再开下一轮。**

唯一的例外是 R2:它**不需要实机验证**,`ToolBench` 就能判定好坏。所以 R2 可以在你玩 R1
的时候由我并行做掉,等你玩完 R1 再一起发。

### 依赖关系(只有三条硬依赖)

```
R1 (D2 拆 switch) ──┬──> R3 (A1/A2 要加 3 个工具)
                    ├──> R4 (A4 要加 1 个工具)
                    └──> R5 (A3 要改 build_shelter)

R2 内部:D1a(翻译)必须在 D1b(删)之前 —— 先把 31% 的安全收益锁定,
        再动内容;而且英文散文比中文更容易看出哪句是重复的。

C3(知识索引)必须和 D1b(删正文)同轮 —— 删掉正文却不给索引,模型会抓瞎。
```
...
我这边每轮的工作量大致是:R1 一轮对话,R2 一轮,R3 一到两轮,R4 两轮(移植 + 改进),R5 一轮。
**真正的瓶颈是你的实机验证** —— R3/R4/R5 都必须进游戏玩过才算数,R1/R2 不用。
