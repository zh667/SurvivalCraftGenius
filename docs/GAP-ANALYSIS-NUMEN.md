...
> **更正(2026-08-13,写完本节后重读代码发现)**:下面这一节把差距写大了。
> `GeniusKnowledge.Read(topic)` 实际上**已经**从磁盘知识文件夹做章节级检索,
> `WriteOrUpgrade` 也**已经**保证玩家编辑过的文件永不被覆盖 —— 玩家是可以自己写攻略的。
> 那 980 行常量是随包发的默认内容 + 升级检测样本,不是唯一来源。
> 真实差距只剩两条:①系统提示词里没有清单(模型得先空调一次才知道有哪些文件);
> ②提示词里还内联了一份和知识文件重复的攻略正文。详见 `docs/PLAN-v0.12.md` §零 与 D1/C3。
> 以下原文保留,作为"我把差距写大了"的记录。
